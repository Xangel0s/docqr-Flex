...
class DocumentController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            // MODO SEGURO: Eliminar select() restrictivo para evitar Error 500
            // Si alguna columna no existe o tiene nombre diferente, el select() explícito causa crash
            // Al usar * (implícito), Laravel trae todas las columnas disponibles y evita errores
            // CRÍTICO: Excluir explícitamente documentos eliminados (soft delete)
            $query = QrFile::withoutTrashed()
                ->whereNotNull('qr_id') // Ignorar documentos sin QR
                ->where('qr_id', '!=', ''); // Ignorar QR vacíos
            ...
            $allowedSorts = ['created_at', 'original_filename', 'scan_count', 'folder_name', 'fecha_emision'];
            if (!in_array($sortBy, $allowedSorts)) {
                $sortBy = 'created_at';
            }
            
            // CRÍTICO: Ordenamiento estable para paginación consistente
            // Agregar 'id' como segundo campo de ordenamiento para evitar resultados inconsistentes
            // cuando hay múltiples registros con el mismo valor en el campo principal
            $query->orderBy($sortBy, $sortOrder);
            if ($sortBy !== 'id') {
                $query->orderBy('id', $sortOrder); // Ordenamiento secundario por ID para estabilidad
            }

            $perPage = min(max(1, (int) $request->input('per_page', 15)), 10000);
            
            // PAGINACIÓN: Usar paginación estándar sin forzar página manual para evitar conflictos
            // withQueryString() mantiene los filtros en los links de paginación
            $documents = $query->paginate($perPage)->withQueryString();
            
            // TRANSFORMACIÓN DE DATOS: Usar through() para mantener estructura de paginación
            // Protección de fechas con optional() para evitar crash si vienen null o con formato extraño
            $documents->through(function ($document) {
                $hasQrId = !empty($document->qr_id) && $document->qr_id !== '';
                
                // Uso de optional() para evitar crash si la fecha es null o inválida
                $created = optional($document->created_at);
                $updated = optional($document->updated_at);
                $scanned = optional($document->last_scanned_at);
                
                $renderMode = $this->resolveRenderMode($document);
                ...
                return [
                    ...
                    // Fechas seguras: usar optional() para evitar "Call to member function format() on null"
                    'last_scanned_at' => ($scanned && method_exists($scanned, 'format')) 
                        ? $scanned->format('Y-m-d H:i:s') 
                        : ($document->last_scanned_at ? (string) $document->last_scanned_at : null),
                    'created_at' => ($created && method_exists($created, 'format')) 
                        ? $created->format('Y-m-d H:i:s') 
                        : ($document->created_at ? (string) $document->created_at : null),
                    'updated_at' => ($updated && method_exists($updated, 'format')) 
                        ? $updated->format('Y-m-d H:i:s') 
                        : ($document->updated_at ? (string) $document->updated_at : null),
                    // URLs: verificación segura antes de generar
                    'view_url' => $hasQrId ? \App\Helpers\UrlHelper::url("/api/view/{$document->qr_id}", request()) : null,
                    'qr_url' => $hasQrId ? \App\Helpers\UrlHelper::url("/api/view/{$document->qr_id}", request()) : null,
                    'pdf_url' => $hasQrId ? \App\Helpers\UrlHelper::url("/api/files/pdf/{$document->qr_id}", request()) : null,
                    'pdf_original_url' => $hasQrId ? \App\Helpers\UrlHelper::url("/api/files/pdf-original/{$document->qr_id}", request()) : null,
                    'qr_image_url' => $hasQrId ? \App\Helpers\UrlHelper::url("/api/files/qr/{$document->qr_id}", request()) : null,
                    'render_mode' => $renderMode,
                    // Verificación segura de final_path (puede no existir en la BD)
                    'final_pdf_url' => ($renderMode === 'embedded' && $hasQrId)
                        ? \App\Helpers\UrlHelper::url("/api/files/pdf/{$document->qr_id}", request())
                        : null,
                ];
            });

            // Generar identificadores únicos ANTES de construir la respuesta
            ...
            return response()->json([
                ...
                'meta' => [
                    ...
                    // URLs de navegación para el Frontend
                    ...
                    'path' => $documents->path(),
                    // Datos Anti-Race Condition
                    'response_id' => $uniqueId,
                    'response_timestamp' => $timestamp,
                    'page_requested_at' => now()->toIso8601String(),
                ]
            ], 200)
            ->header('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0, private')
            ->header('Pragma', 'no-cache')
            ->header('Expires', '0')
            ->header('X-Request-ID', $uniqueId)
            ->header('X-Page-Number', (string) $currentPage)
            ->header('X-Total-Pages', (string) $documents->lastPage())
            ->header('Vary', 'Accept, Accept-Encoding, X-Requested-With');

        } catch (\Exception $e) {
            // Loguear el error real es vital para saber qué pasó
            Log::error('CRASH en index: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error interno del servidor. Verifique logs.'
            ], 500);
        }
    }
        ...
        try {
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'view_url' => \App\Helpers\UrlHelper::url("/api/view/{$document->qr_id}", request()),
                    ...
                    'render_mode' => $renderMode,
                    'final_pdf_url' => $renderMode === 'embedded'
                        ? \App\Helpers\UrlHelper::url("/api/files/pdf/{$document->qr_id}", request())
                        : null,
                ]
            ], 200)
    ...
    public function showByQrId(string $qrId): JsonResponse
    {
        try {
            ...
            $document = QrFile::withoutTrashed()->select([
                ...
                'created_at', 'updated_at', 'final_path', 'file_path', 'qr_path', 'fecha_emision'
    ...
    private function resolveRenderMode(QrFile $document): string
    {
        if (!empty($document->final_path)) {
            return 'embedded';
        }

        if (!empty($document->qr_position) && $document->status === 'completed') {
            return 'overlay';
        }
