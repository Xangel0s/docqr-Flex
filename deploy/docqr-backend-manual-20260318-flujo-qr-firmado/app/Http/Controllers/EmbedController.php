...
class EmbedController extends Controller
{
    ...
    public function __construct(PdfProcessorService $pdfProcessor, PdfSignatureService $pdfSignatureService)
    {
    ...
    public function embed(Request $request): JsonResponse
    {
        try {
            ...
            $sourceFullPath = $pdfDiskToUse === 'final'
                ? Storage::disk('final')->path($pdfPathToUse)
                : Storage::disk('local')->path($pdfPathToUse);
            $signatureCheck = $this->pdfSignatureService->analyzePath($sourceFullPath, $qrFile->original_filename);
            ...
            if ($signatureCheck['signed']) {
                return $this->storeOverlayPosition($qrFile, $request, array_merge($position, ['page_number' => $pageNumber]), [
                    'context' => 'embed',
                    'source_path' => $pdfPathToUse,
                    'disk' => $pdfDiskToUse,
                    'markers' => $signatureCheck['markers'],
                ]);
            }
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'view_url' => \App\Helpers\UrlHelper::url("/api/view/{$qrFile->qr_id}", $request),
    ...
    public function embedPdf(Request $request): JsonResponse
    {
        try {
            ...
            $sourceFullPath = null;
            $signatureCheck = ['signed' => false, 'markers' => []];
            if ($qrFile->file_path && Storage::disk('local')->exists($qrFile->file_path)) {
                $sourceFullPath = Storage::disk('local')->path($qrFile->file_path);
            } elseif ($qrFile->final_path && Storage::disk('final')->exists($qrFile->final_path)) {
                $sourceFullPath = Storage::disk('final')->path($qrFile->final_path);
            }
            ...
            if (!empty($signatureCheck['signed'])) {
                return $this->storeOverlayPosition($qrFile, $request, array_merge($position, [
    ...
    private function storeOverlayPosition(QrFile $qrFile, Request $request, array $position, array $context = []): JsonResponse
    {
        Log::info('PDF firmado detectado: guardando QR en modo overlay', [
            'qr_id' => $qrFile->qr_id,
        ...
        DB::transaction(function () use ($qrFile, $position) {
            $qrFile->update([
                'final_path' => null,
                'qr_position' => $position,
                'status' => 'completed',
        ...
        return response()->json([
            'success' => true,
            'message' => 'El QR se alojó sin modificar el PDF firmado. La firma original se conserva y el QR se mostrará en la vista pública.',
            'data' => [
                'final_pdf_url' => null,
                'view_url' => \App\Helpers\UrlHelper::url("/api/view/{$qrFile->qr_id}", $request),
                'render_mode' => 'overlay',
